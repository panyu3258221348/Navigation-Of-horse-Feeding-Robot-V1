...
Node(
        ...
        Node(
            ...
            parameters=[{
              ...
              'Mem/STMSize': '45',
              'Mem/LaserScanNormalK': '8',
              'Mem/RehearsalSimllarity': '0.6',
              'Reg/Strategy': '1',
              'Icp/PM': True,
              'Icp/VoxelSize': '0.1',
              'Icp/PointToPlaneK': '5',
            ...
            remappings=[
              ('scan_cloud', '/output_filter'),
              ('imu', '/synced_imu'),  # 使用同步后的IMU数据
              ('odom', '/synced_odom')  # 使用同步后的里程计数据
            ]
            ), 
       
        # 启动时间同步节点
        Node(
            package='rover_slam', executable='time_sync_node', output='screen',
            name='time_sync_node',
            parameters=[{
                'use_sim_time': False
            }]
